...
# modified from original source: https://github.com/neilk/octopress-flickr
...
class FlickrPhotoHtml
  ...
  def toHtml
    ...
    figureClass = ['thumbnail flickr']
    unless @klass.nil? or @klass.empty?
      figureClass.push(@klass)
    end
    figureAttrs = { 'class' => figureClass.join(" "), 'markdown' => 0 }
    figureCssAttrs = {}
    ...
    unless @width.nil?
      imgCssAttrs['width'] = @width.to_s + 'px';
      figureCssAttrs['width'] = (@width.to_i+10).to_s + 'px';
      # unless @height.nil?
    ...
    x.div(figureAttrs) { |x|
      ...
      x.div(captionAttrs) { |x|
        x.span({'class' => 'description'}) { |x|
          x << @desc
        }
        x << " "
        x.div('class' => 'pull-right') { |x|
          if @username
            x << "By "
            x.a('class' => 'flickr-link', 'href' => @page_url) { |x| x << @username }
          else
            x << "From "
            x.a('class' => 'flickr-link', 'href' => @page_url) { |x| x << "Flickr" }
          end
...
begin
  FlickRaw.api_key = ENV['FLICKR_API_KEY'] || data.site.flickr.api_key
  FlickRaw.shared_secret = ENV['FLICKR_API_SECRET'] || data.site.flickr.shared_secret
rescue
...
module FlickrHelpers
  def flickr_tag(id, options)
    FlickrImageTag.new(id, options).render
  end
end